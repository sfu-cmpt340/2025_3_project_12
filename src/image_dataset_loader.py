...
def _load_indices(csv_path: Path) -> list:
    """
    Helper function to convert indices of images to use for phase of AI pipeline (as suggested by SFU dataset) to list.
...
def _get_inception_v3_image_transform() -> transforms.Compose:
    """
    Helper function that creates image transformation for Inception V3 model. The model requires 299x299 images.
...
class MelanomaImageDatasetLoader:
    ...
    def __init__(self, metadata_csv_path: Path, images_zip_path: Path):
        """
